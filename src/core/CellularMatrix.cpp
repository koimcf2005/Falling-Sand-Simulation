...
void CellularMatrix::activateChunkAt(int worldX, int worldY) {
    ...
    activateNeighboringChunks(chunkX, chunkY);
}

void CellularMatrix::activateNeighboringChunks(int chunkX, int chunkY) {
    // Activate all 8 neighboring chunks
    for (int dy = -1; dy <= 1; ++dy) {
        for (int dx = -1; dx <= 1; ++dx) {
            if (dx == 0 && dy == 0) continue; // Skip center chunk
            int neighborX = chunkX + dx;
            int neighborY = chunkY + dy;
            if (isValidChunk(neighborX, neighborY)) {
                chunks[neighborY][neighborX].setActiveNextFrame();
            }
        }
    }
}
